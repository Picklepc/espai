...
;   $env:ESPAI_HUB_URL   = "http://espai.local:7888"  (or your hub IP)
...
build_flags =
  ...
  -D HUB_URL=\"${{sysenv.ESPAI_HUB_URL}}\"
 ...
 * {name} — ESPAI Project Firmware
 *
 * Built on the ESPAI seed firmware platform. Patterns available:
 *   hubCheckinFull()          — POST identity + config schema to hub on boot
 *   espai_poll_commands()     — Poll hub for pending commands (call from loop)
 *   espai_register_config()   — Declare a configurable NVS key (hub ⚙ Settings)
 *   espai_init_config()       — Apply all registered NVS keys at boot
 *   espai_upload_jpeg()       — Upload JPEG buffer to hub media store
 *   enterDeepSleep(seconds)   — Deep sleep with RTC timer wake
 *
 * Build flags required in platformio.ini:
 *   -D WIFI_SSID / WIFI_PASS  — credentials (use sysenv, never hardcode)
 *   -D HUB_URL                — e.g. "http://espai.local:7888"
 *   -D NODE_NAME / BOARD_ID / FW_VERSION
 */
...
// ── Node ID (hashed MAC — never expose raw MAC) ──────────────────────────
String deriveNodeId() {{
  ...
  mbedtls_md_starts(&ctx); mbedtls_md_update(&ctx, mac, 6); mbedtls_md_finish(&ctx, hash);
  mbedtls_md_free(&ctx);
  String id = "node-";
...
// ── Hub data push — call this to send sensor readings to the project store ─
// Replace the example fields with your actual sensor values.
void pushToHub(float sensorValue) {{
  if (!wifiConnected) return;
#ifdef HUB_URL
  HTTPClient http;
  http.begin(String(HUB_URL) + "/api/projects/YOUR_PROJECT_ID/data");
  http.addHeader("Content-Type", "application/json");
  http.addHeader("X-Device-ID", nodeId);
  // Build a multi-field JSON payload — add all your sensor readings here
  String body = "{{\\\"value\\\":" + String(sensorValue, 2) + "}}";
  http.POST(body);
  http.end();
#endif
}}
...
void handleStatus() {{
  ...
  doc["ip"]        = wifiConnected ? WiFi.localIP().toString() : "0.0.0.0";
  sendJson(200, doc);
}}

void handleReboot() {{
  JsonDocument doc; doc["status"] = "rebooting"; sendJson(200, doc);
  delay(200); ESP.restart();
}}

void handleNotFound() {{
  JsonDocument doc; doc["error"] = "not found"; sendJson(404, doc);
}}

// ── TODO: add your project-specific endpoints here ────────────────────────
...
void setup() {{
  Serial.begin(115200);
  delay(100);
  Serial.println("[{node_name}] ESPAI firmware " FW_VERSION);

  WiFi.mode(WIFI_STA);
  nodeId = deriveNodeId();

  // ── Register configurable NVS settings (hub shows these in ⚙ Settings) ──
  // espai_register_config("my_setting", "int", "0",
  //   "Description of what this setting does",
  //   [](const char* v) {{ /* apply new value */ }});
  // espai_init_config();  // reads NVS + fires callbacks once at boot

  // ── Connect WiFi (NVS credentials first, then build flags, then AP mode) ─
  Preferences prefs;
  ...
  if (WiFi.status() == WL_CONNECTED) {{
    ...
    // Fallback AP so you can configure WiFi without a serial cable
  ...
  // TODO: server.on("/api/sensor", HTTP_GET, handleSensorRead);
  server.onNotFound(handleNotFound);
  server.begin();
  Serial.println("[{node_name}] HTTP server started on port 80");

  // Hub checkin — posts identity + config schema; reads back sleep intervals
#ifdef HUB_URL
  if (wifiConnected) hubCheckinFull(String(HUB_URL));
#endif
}}
...
void loop() {{
  ...
  // Poll hub for pending commands every 2 s (reboot, set_config, user actions)
#ifdef HUB_URL
  if (wifiConnected) espai_poll_commands(String(HUB_URL));
#endif
...
### Push readings from firmware (multi-field payload)
```cpp
void pushToHub(float temp, float humidity, int battery_mv) {{
  if (WiFi.status() != WL_CONNECTED) return;
  HTTPClient http;
  http.begin("http://espai.local:7888/api/projects/{project_id}/data");
  http.addHeader("Content-Type", "application/json");
  http.addHeader("X-Device-ID", nodeId);
  // Build any JSON payload — all fields land in the project data store
  JsonDocument doc;
  doc["temp_c"]      = temp;
  ...
  http.end();
  if (code != 201) Serial.printf("[hub] push failed: %d\\n", code);
}}
```
Add to `platformio.ini` build_flags: `-D HUB_PROJECT_ID=\\"{project_id}\\"`

### Receiving commands from hub (command channel)
```cpp
// Register a user command handler BEFORE espai_poll_commands is called.
// Return true if you handled the command, false to fall through to built-ins.
void setup() {{
  espai_register_cmd_handler([](const String& id, const String& type, const JsonObject& pl) {{
    if (type == "set_output") {{
      int pin = pl["pin"] | 0;
      bool val = pl["value"] | false;
...
void loop() {{
  // Poll hub every ESPAI_CMD_POLL_MS (default 2 s); dispatches reboot,
  // set_config, run_ota_check, and your custom handlers above.
#ifdef HUB_URL
  espai_poll_commands(String(HUB_URL));
#endif
}}
```
Send commands from hub: `PUT /api/devices/{{device_id}}/commands` or via Rules engine action `send_command`.
...
export ESPAI_WIFI_SSID="MyNetwork" ESPAI_WIFI_PASS="mypassword" ESPAI_HUB_URL="http://espai.local:7888"
pio run                   # compile (fix all errors before committing)
pio run -t upload         # compile + flash via USB
pio device monitor        # serial monitor
# After successful build, import to OTA catalog:
# POST /api/projects/{project_id}/import-build
```
...
- Hub checkins are fire-and-forget in `hubCheckinFull()` — never block the main loop
- AP fallback required: `ESPAI-{{node_id_suffix}}` hotspot when WiFi fails — scaffolded in main.cpp
- Never register `sta_ssid`, `sta_pass`, `sleep_s`, `awake_s`, or `espai_*` keys — platform-managed

## Hub Workers (project-scoped processing)

Workers in `workers/` run on the hub and process data, call APIs, or trigger events.
For project-specific customization, place workers in `projects/{project_id}/workers/{{name}}/`
(the job runner checks there first and falls back to the global `workers/` directory).

### Pattern: process data pushed by device
```python
# workers/my-processor/main.py
import json, os, sys

HUB_URL    = os.environ.get("ESPAI_HUB_URL", "http://localhost:7888")
PROJECT_ID = os.environ.get("ESPAI_PROJECT_ID", "{project_id}")

def run(inputs: dict) -> dict:
    # Read latest device data
    import urllib.request
    with urllib.request.urlopen(f"{{HUB_URL}}/api/projects/{{PROJECT_ID}}/data/latest") as r:
        data = json.loads(r.read())
    # Process ...
    result = {{"processed": True, "input_keys": list(data.keys())}}
    # Emit events to trigger rules engine
    return {{"result": result, "events": [{{"type": "analysis.complete", "data": result}}]}}
```

Test workers from the hub UI with the **▶ Test** button on each worker card (passes JSON inputs, returns outputs + stderr).
"""
